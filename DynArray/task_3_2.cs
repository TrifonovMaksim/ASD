...
namespace AlgorithmsDataStructures
{
    ...
    public class BankDynArray<T>
    {
        ...
        public void Remove(int index)
        {
            ...
            count -= 1;
            if (capacity / 1.5 >= count && capacity > 16)
            {
     ...
     * По времени и по памяти сложность для банковских операций O(1).
     */
     ...
     * Сложность по времени, для операция, это сложность одномерного массива + уровень глубины индексации.
     * Сложность по памяти, для каждой глубины свой массив, если считать глубина за n тогда O(n).
     */


    /*8. Функция суммирования двух связанных списков.
     * Рефлексия по прошлому заданию, я сделал почти, как в эталонном решении, проверил равенство длин, но не выдавал ошибку а просто возвращал пустой список. Может лучше возвращать какой нибудь null, но это надо уже в том месте где будет использоваться функция данная, правильно реализовать обработку выходных результатов, чтобы сразу понятно было что длины не равны.
     * Также как в примере шел по 2 спискам одновременно, и добавлял в конец, но в отличии от эталона, проверял 2 списка зачем то) хотя можно было один раз длины равны.
     */
}
